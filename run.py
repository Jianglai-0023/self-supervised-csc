...
class ConfusDataset(IterableDataset):
    ...
    def __init__(self, list_of_files, tokenizer, max_seq_length, seed, confuse_rate, buffer_size=10000, shuffle=True):
        super().__init__()
        ...
        for filename in self.list_of_files:
            with open(filename) as f:
                for line in f:
                    ...
                    trg = stdf(trg).replace(" ","")
                    src = stdf(src).replace(" ","")

                    if len(src)==len(trg):
                        if random.random() < self.confuse_rate: 
                            src, trg = self.confus(trg)
                        else:
                            src = list(src)
                            trg = list(trg)
...
class DataProcessor:
    ...
    def get_train_examples(self, data_dir, filename, mnlg=False, ann_rate = 0):
        return self._create_examples(self._read(os.path.join(data_dir, filename)), "train", mnlg, ann_rate)
    ...
    def _create_examples(self, lines, set_type, mnlg=False, ann_rate = 0):
        examples = []
            ...
            if len(src) == len(trg):     
                if mnlg:
                    ...
                    if rate < ann_rate:
                        ...
                        examples.append(InputExample(guid=guid, src=trg, trg=trg))   
                else: 
                    examples.append(InputExample(guid=guid, src=src, trg=trg))
        return examples
    ...
    def convert_examples_to_features(examples, max_seq_length, tokenizer, verbose=True):
        ...
        for i, example in enumerate(examples):
            ...
            if offset_length > 0:
                ...
                attention_mask = attention_mask + [0] * offset_length
                label_ids = label_ids + [tokenizer.pad_token_id] * offset_length
                ref_ids = ref_ids + [tokenizer.pad_token_id] * (max_seq_length-len(ref_ids))
...
def mask_tokens_only_neg(inputs, labels, tokenizer, noise_probability=0.2,masked = False):
    inputs = inputs.clone()
    probability_matrix = torch.full(inputs.shape, noise_probability)
    special_tokens_mask = [
        tokenizer.get_special_tokens_mask(val, already_has_special_tokens=True) for val in inputs.tolist()
...
def main():
    ...
    # Data config
    parser.add_argument("--data_dir", type=str, default="/home/jiangl/csc",
    ...
    parser.add_argument("--test_on_lemon", action="store_true",
    ...
    parser.add_argument("--model_type", type=str, default="mdcspell",
    ...
    parser.add_argument("--mnlg", action="store_true",
                        help="Training with monolingual data.")
    parser.add_argument("--adv",action="store_true",
                        help="Adversarial noise")
    parser.add_argument("--do_confuse",action="store_true",
                        help="use confusion process data")
    parser.add_argument("--confuse_rate",type = float, default=1,
                        help="confuse part of the train data")
    parser.add_argument("--masked",action="store_true",
                        help="use [MASK]")
    parser.add_argument("--calc_prd",action="store_true",
                        help="calculate prd_ids instead of output directely")
    parser.add_argument("--anno_rate",type = float, default = 0)
    ...
    output_args_file = os.path.join(args.output_dir,"args")
    with open(output_args_file,"w") as writer:
        for arg in vars(args):
            writer.write(f"{arg}: {getattr(args, arg)}" + "\n")
    ...
    if args.do_train:
        ...
        if args.do_confuse:
            dataset = ConfusDataset(os.path.join(args.data_dir, args.train_on), tokenizer, args.max_seq_length, seed=args.seed, shuffle=True,confuse_rate=args.confuse_rate)
            train_dataloader = DataLoader(dataset, batch_size=args.train_batch_size, num_workers=0)
            train_dataloader = accelerator.prepare(train_dataloader)
        else: 
            train_examples = processor.get_train_examples(args.data_dir, args.train_on, args.mnlg, args.anno_rate)
    ...
    def _inner_update(delta):
        ...
        delta = (delta + adv_lr * delta_grad / grad_norm).detach()
        ...
        for epoch in range(int(args.num_train_epochs)):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                if relm:
                    ...
                    if args.mft:
                        src_ids = mask_tokens_only_neg(src_ids, trg_ref_ids, tokenizer, args.noise_probability,masked = args.masked)
                else:
                    src_ids, attention_mask, trg_ids = batch
                    if args.mft:
                        src_ids = mask_tokens_only_neg(src_ids, trg_ids, tokenizer, args.noise_probability,masked = args.masked)
